...
var maximo;
var contagem = 0
var atualizacao_automatica = false
...
document.getElementById('atualizacao').addEventListener('change', function () {
    if (this.value == 'Ligado') {
...
setInterval(function () {
    if (atualizacao_automatica) {
        document.getElementById('input_qtde').value = 1
        atualizar_dados_api()
    }
}, 600000)
...
function mensagem_carregamento(mensagem) {
    ...
    var label_carregando = document.createElement('label')
    label_carregando.id = 'label_carregando'
    label_carregando.textContent = '0%'
    label_carregando.style.color = '#ffffff'
    label_carregando.id = 'carregamento'

    label_aguarde.textContent = mensagem

    div_aguarde.append(img, label_aguarde, label_carregando)
...
async function atualizar_dados_api() {
    ...
    // Lógica das regiões; Precia fazer a requisição 6 vezes, 1 para cada região cadastrada (Nordeste está duplicado);
    var regioes = {
        "REGIÃO CENTRO OESTE": 2433,
        "REGIÃO NORDESTE": 2162,
        "REGIÃO NORTE": 2434,
        "REGIÃO SUDESTE": 2421,
        "REGIÃO SUL": 2371
    }

    Object.keys(regioes).forEach(async function (regiao) {
        mensagem_carregamento('Sincronizando chamados das regiões...')

        localStorage.removeItem('chamados_por_regiao')
        await buscar_ocorrencias_por_regiao(regioes[regiao], regiao)
    })
...
async function buscar_ocorrencias_por_regiao(codigo_regiao, nome_regiao) {
    return new Promise((resolve, reject) => {

        var paginas = Number(document.getElementById('input_qtde').value)

        for (let i = 1; i <= paginas; i++) {
            fetch(`https://lighthousev2.lkp.app.br/v1/ocorrencias?GrupoUnidadeId=${codigo_regiao}&PageIndex=${i}&PageSize=1000`, {
                method: 'GET',
                headers: {
                    'accept': 'text/plain',
                    'EmpresaId': '5795',
                ...
                .then(data => {
                    var chamados_por_regiao = JSON.parse(localStorage.getItem('chamados_por_regiao')) || {}

                    data = JSON.parse(data)

                    Object.keys(data).forEach(chamado => {
                        chamados_por_regiao[data[chamado].cqa] = nome_regiao
                    })

                    localStorage.setItem('chamados_por_regiao', JSON.stringify(chamados_por_regiao))
...
async function tipoCorrecoes() { //29
    return new Promise((resolve, reject) => {
...
async function pesquisar() {
    ...
    trs.forEach((tr) => {
        ...
        var mesmo_regiao = listagens.regiao.length === 0 || listagens.regiao.includes(String(chamado.regiao || '').trim());

        // Se algum filtro não for atendido, mantém oculto
        if (mesmo_statusTexto && mesmo_executor && mesmo_solicitante && mesmo_tipoCorrecaoNome && mesmo_regiao) {
...
function api_ocorrencias(pagina) {
    return new Promise((resolve, reject) => {
        ...
        const url = `https://lighthousev2.lkp.app.br/v1/ocorrencias?PageIndex=${pagina}&PageSize=1000`;
        const headers = {
            'accept': 'text/plain',
            'EmpresaId': '5795',
            'Authorization': 'Bearer ' + autorizacao
...
async function buscar_ocorrencias() {
    ...
    var pagina = Number(document.getElementById('input_qtde').value)
    progresso = 0
    maximo = pagina * 1000
    contagem = 0
            ...
            const promises = data.map(async (item) => {
                try {
                    ...
                    ocorrencias[item.cqa] = {
                        ...
                        'regiao': chamados_por_regiao[item.cqa],
...
async function lancar_na_pagina() {
    ...
    var divs = ['solicitante', 'executor', 'tipoCorrecaoNome', 'statusTexto', 'regiao']
