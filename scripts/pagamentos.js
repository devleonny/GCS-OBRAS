...
async function abrir_detalhes(id_pagamento) {
    ...
    Object.keys(pagamento.anexos).forEach(anx => {
        ...
        let arquivo = `https://drive.google.com/file/d/${anexo.link}/view?usp=drivesdk`
        ...
        for (let campo in campos) {
            ...
            if (campo == 'pedido') {
                ...
                if (orcamento.status && orcamento.status[pagamento.id_pedido] && orcamento.status[pagamento.id_pedido].historico) {
                    ...
                    let his = orcamento.status[pagamento.id_pedido].historico

                    for (let hist in his) {

                        let item_historico = his[hist]

                        if (String(item_historico.status).includes('ANEXADO')) {

                            let anexos = item_historico.anexos

                            for (let anx in anexos) {
...
function editar_resumo(id_pagamento) {
    ...
    let container_resumo = document.getElementById('container_resumo')
...
function colorir_parceiros() {
    ...
    if (detalhes) {
        ...
        let labels = detalhes.querySelectorAll('label.numero')
        let divs = detalhes.querySelectorAll('div.container')
...
function alterar_centro_de_custo(id) {
    ...
    let centro_de_custo_div = document.getElementById('centro_de_custo_div')
