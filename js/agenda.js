...
const regioes = ['Nordeste', 'Norte', 'Sudeste', 'Sul', 'Centro-Oeste', 'Sem Região', 'Clientes Novos', 'Administrativo']

async function iniciar_agendas(alterar) {
    await telaAgenda(alterar)
}
...
async function telaAgenda(alterar) {
    ...
    if (alterar) {
        ...
        localStorage.setItem('periodoAtual', JSON.stringify({
    ...
    const periodoAtual = JSON.parse(localStorage.getItem('periodoAtual')) || false
    const referencia = periodoAtual || { mes: dataAtual.getMonth(), ano: dataAtual.getFullYear() }
    selectAno = referencia.ano
    selectMes = referencia.mes

    dias = new Date(selectAno, meses[selectMes] + 1, 0).getDate()
    let ths = `
    ...
    for (let i = 1; i <= dias; i++) { // Cabeçalho dias/Semanas do mês
        let data = new Date(selectAno, meses[selectMes], i)
    ...
    const regioesString = regioes.map(regiao => `
    ...
    <div class="filtrosAgenda">
        <div style="${horizontal}; gap: 5px;">
            <select id="mes" onchange="iniciar_agendas(true)">
                ${Object.entries(meses).sort().map(([mes, nome]) => `<option value="${mes}" ${mes === selectMes ? 'selected' : ''}>${mes}</option>`).join('')}
            </select>

            <select id="ano" onchange="iniciar_agendas(true)">
                ${['2024', '2025', '2026'].map(op => `<option ${selectAno == op ? 'selected' : ''}>${op}</option>`).join('')}
            </select>
        </div>

        <input class="campos" oninput="filtrar_por_regiao()" id="campo_tecnico_pesquisa" placeholder="Nome Funcionário">
    ...
    `
    menus.insertAdjacentHTML('beforeend', acumulado)

    for (const [omieFuncionario, funcionario] of Object.entries(dados_agenda_tecnicos)) criarLinhaAgenda(omieFuncionario, funcionario)
...
function criarLinhaAgenda(omieFuncionario, funcionario) {
    ...
    let celulas_dias = ''
    let agenda = {}
    let periodo = `${selectAno}_${meses[selectMes]}`
    ...
    const regiao = dados_agenda_tecnicos[omieFuncionario]?.regiao_atual || 'Sem Região'

    if (acesso.permissao !== 'adm' && regiao == 'Administrativo') return

    const linha = `
        <tr ${omieFuncionario}>
            <td class="drag-handle">☰</td>
            <td id="tr_${omieFuncionario}" style="text-align: left;">${funcionario.nome || omieFuncionario}</td>
            <td>
        ...
        `
    const linhaExistente = document.getElementById(`tr_${omieFuncionario}`)

    if (linhaExistente) return linhaExistente.innerHTML = linha
    document.getElementById('bodyAgenda').insertAdjacentHTML('beforeend', linha)
}
